...
def labhistogram(nbbins):
    """ Computes lab histogram of an image, with each bin normalized to the [0;1]
        range, as a feature vector. Curried arguments.
    
    Arguments:
        labimg LAB image to compute the color histogram of. Should be a float32
               LAB image as defined by OpenCV's cvtColor documentation, i.e.
               L is between 0 and 100, a and b between -127 and 127 (all bounds
               inclusive).
        nbbins 3-tuple containing the number of bins per channels.

    Returns:
        A feature vector of size prod(nbbins) where each bin is normalized to the
        [0;1] range - e.g. each bin contains a probability of a random pixel in the
        patch belonging to the bin (so it all sums to 1).
    """
    return (lambda img: (colorhistogram(img, 
                                        nbbins, 
                                        [0,101,-127, 128, -127, 128]).astype(np.float64)
                         / np.prod(img.shape[0:2])).flatten('C'))

def labhistogramviz(hist):
    """ Visualize a lab histogram as linear combination of bin median colors
        weighted by occurences. Returns a lab image.
    """
